...
function renderMarket() {
  ...
  const withBook = curIss.filter(b => !PL.named.has(b.name)).filter(b => { const A1 = anchorFor(b.name); return A1 && A1.investors && A1.investors.length; }), pending = curIss.filter(b => !withBook.includes(b) && !PL.named.has(b.name) && !/SME/i.test(b.type));
  $("#anchorCards").innerHTML = PL.html + withBook.map(b => { const A1 = anchorFor(b.name), q = anchorScore(A1); const tot = A1.investors.reduce((s, x) => s + (x.pct || 0), 0);
