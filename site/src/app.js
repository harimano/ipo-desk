...
function renderSheet() {
  ...
  if (kind === "q") {
    ...
    <div class="sec grid g2"${rec.claude && ((rec.claude.bull || []).length || (rec.claude.bear || []).length) ? "" : " hidden"}><div class="card"><div class="ch up">For</div><div class="cb">${li(s.bull)}</div></div><div class="card"><div class="ch down">Against</div><div class="cb">${li(s.bear)}</div></div></div>
...
function renderMarket() {
  ...
  const bookNames = x => (x.investors && x.investors.length) ? x.investors.map(i => i.name) : x.auto ? [] : namesOf(x.anchor);
  const ab = A.map(x => ({ n: x.name, v: x.amountCr != null ? x.amountCr : amt(x.anchor), s: (n => n ? n + (n > 1 ? " names" : " name") : x.issueSizeCr && x.amountCr ? Math.round(x.amountCr / x.issueSizeCr * 100) + "% of issue" : "")(x.count || bookNames(x).length) })).filter(r => r.v != null).sort((x, y) => y.v - x.v);
  ...
  A.forEach(x => bookNames(x).forEach(n => bump(n, 1)));
  ...
  $("#anchorCards").innerHTML = withBook.map(b => { const A1 = anchorFor(b.name), q = anchorScore(A1); const tot = A1.investors.reduce((s, x) => s + (x.pct || 0), 0);
    ...
    + (pending.length ? `<div class="card acard pend" style="grid-column:1/-1;display:block"><div class="lbl" style="margin-bottom:6px">Book size and lock-in dates · investor list not captured</div><div class="chips" style="margin-top:0">${pending.map(b => { const A1 = anchorFor(b.name), f = A1 ? [A1.amountCr ? cr(A1.amountCr) : null, A1.amountCr && A1.issueSizeCr ? Math.round(A1.amountCr / A1.issueSizeCr * 100) + "% of issue" : null, A1.lockIn30 ? "lock-in ends " + fmtD(A1.lockIn30) + " / " + fmtD(A1.lockIn90) : null].filter(Boolean) : []; return `<span title="${esc(A1 && A1.note || b.type)}">${esc(b.name)} <b class="dim">${f.length ? esc(f.join(" · ")) : A1 && A1.date ? "anchor " + fmtD(A1.date) : b.open ? "anchor " + fmtD(iso(new Date(d(b.open).getTime() - DAY))) : "TBA"}</b></span>`; }).join("")}</div><div class="dt" style="margin-top:8px">Size is NSE's stated anchor portion × the upper band; dates are InvestorGain's. Who took each book is published only as the issuer's letter (a PDF), so it is scored only when someone has read it in.</div></div>` : "")
