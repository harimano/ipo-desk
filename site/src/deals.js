...
function renderListingDeals() {
  ...
  const C = $("#ldClients"); if (C) C.innerHTML = `<thead><tr><th>Client</th><th class="r">Deals</th><th class="r">Stocks</th><th>Behaviour</th><th class="r">Net</th></tr></thead><tbody>${rep.map(c => `<tr><td class="nm${follows(c.client) ? " up" : ""}" title="${esc(c.client)}">${esc(short(c.client).slice(0, 28))} ${fb(c.client)}</td><td class="r num">${c.n}</td><td class="r num">${c.stocks}</td><td class="dt" style="white-space:nowrap">${[c.rt ? `${c.rt} round-trip` : "", c.nb ? `<span class="up">${c.nb} net buy</span>` : "", c.ns ? `<span class="down">${c.ns} net sell</span>` : ""].filter(Boolean).join(" · ")}</td><td class="r num ${cls(Math.abs(c.b - c.s) < 0.5 ? 0 : c.b - c.s)}">${ldSigned(c.b - c.s)}</td></tr>`).join("") || `<tr><td colspan="5" class="empty">No name appears twice under this filter.</td></tr>`}</tbody>`;
}
