...
function renderListingDeals() {
  ...
  T.innerHTML = `<thead><tr><th>Date</th><th>Stock</th><th>Client</th><th>Side</th><th class="r">Qty</th><th class="r">Price</th><th class="r">vs issue</th><th class="r">Value</th></tr></thead><tbody>${rows.slice(0, LD_ROWS).map(r => `<tr>
    ...
    <td class="dt" title="${esc(r.client)}">${esc((r.client || "").slice(0, 34))}${(r.client || "").length > 34 ? "…" : ""}</td>
  ...
  const C = $("#ldClients"); if (C) C.innerHTML = `<thead><tr><th>Client</th><th class="r">Deals</th><th class="r">Stocks</th><th class="r">Bought</th><th class="r">Sold</th><th class="r">Net</th></tr></thead><tbody>${rep.map(c => `<tr><td class="nm" title="${esc(c.client)}">${esc((c.client || "").slice(0, 32))}${(c.client || "").length > 32 ? "…" : ""}</td><td class="r num">${c.n}</td><td class="r num">${c.stocks.size}</td><td class="r num">${cr(c.b)}</td><td class="r num">${cr(c.s)}</td><td class="r num ${cls(Math.abs(c.b - c.s) < 0.5 ? 0 : c.b - c.s)}">${ldSigned(c.b - c.s)}</td></tr>`).join("") || `<tr><td colspan="6" class="empty">No name appears twice under this filter.</td></tr>`}</tbody>`;
}
